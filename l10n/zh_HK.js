OC.L10N.register(
    "gptfreeprompt",
    {
    "Text generation ready" : "文字產生就緒",
    "View" : "檢視",
    "Delete" : "刪除",
    "AI text generation" : "AI 文字產生",
    "FreePromptTaskType not available" : "FreePromptTaskType 不可用",
    "Failed process prompt" : "處理提示失敗",
    "Failed to run or schedule a task" : "執行或安排任務失敗",
    "Failed to create a generation" : "創建產生失敗",
    "Failed to get prompt history" : "取得提示歷史紀錄失敗",
    "Success" : "成功",
    "Failed" : "失敗了",
    "Processing" : "處理中",
    "Deleted" : "已刪除",
    "Unknown" : "不詳",
    "Generation not found" : "找不到產生",
    "Failed to retrieve outputs" : "移除輸出失敗",
    "Failed to cancel generation" : "取消產生失敗",
    "Failed to set notification: Unknown user" : "設定通知失敗：未知的使用者",
    "Failed to set notification: Generation not found" : "設定通知失敗：找不到產生",
    "Failed to set notification: Unknown error" : "設定通知失敗：未知的錯誤",
    "GptFreePrompt" : "GptFreePrompt",
    "A Smart Picker for interfacing with the GPT integrations that provide free prompt cabability" : "用於與 GPT 整合互動的智慧型挑選程式，提供免費的提示功能",
    "A unified Smart Picker for Nextcloud large language model integrations that provide free prompting cabability." : "Nextcloud 大型語言模型整合的統一智慧型挑選程式，提供免費的提示功能。",
    "Text generation content" : "文字產生內容",
    "The text generation task was scheduled to run in the background." : "文字產生任務已安排於背景執行。",
    "Estimated completion time: " : "預計補齊時間：",
    "This can take a while…" : "這可能需要一段時間…",
    "Some generations are still being processed in the background! Showing finished generations." : "部份產生仍在背景處理！顯示已完成的產生。",
    "Loading generations…" : "正在載入產生…",
    "Copy text to clipboard" : "複製文字到剪貼板",
    "Reset" : "重設",
    "Include prompt" : "包含提示",
    "Unexpected server response" : "未預期的伺服器回應",
    "This generation has been deleted." : "此產生已被刪除。",
    "The processing of generations failed." : "產生處理失敗。",
    "The processing of some generations failed." : "部份產生處理失敗。",
    "Text generation error" : "文字產生錯誤",
    "Unknown text generation API error" : "未知的文字產生 API 錯誤",
    "Prompt" : "提示詞",
    "Result" : "結果",
    "Result {index}" : "結果 {index}",
    "ERROR: No free prompt provider available" : "錯誤：沒有可用的免費提示提供者",
    "Show/hide advanced options" : "顯示/隱藏進階選項",
    "Advanced options" : "進階選項",
    "Preview text generation by AI" : "預覽人工智慧產生的文字",
    "Notify when ready" : "就緒時通知",
    "Submit text generated by AI" : "遞交人工智慧產生的文字",
    "Done" : "完成",
    "How many results to generate" : "要產生多少結果",
    "What is the matter with putting pineapple on pizza?" : "把菠蘿放在披薩上是怎麼回事？",
    "Regenerate" : "重新生成",
    "Preview" : "預覽",
    "You will be notified when the text generation is ready." : "文字產生就緒時將會通知您。",
    "Notify when ready error" : "就緒時通知錯誤",
    "Unknown notify when ready error" : "未知的就緒時通知錯誤",
    "Text generation" : "產生文字"
},
"nplurals=1; plural=0;");
